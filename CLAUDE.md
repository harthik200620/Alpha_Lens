...
| `MOMENTUM_CONFIRM_ENABLED` / `MOMENTUM_CONFIRM_MIN_R` | 0 / 0.0 | **Momentum-confirmation gate — the evidence-backed INVERSE of the unreacted gate.** `/api/admin/eval-cuts` over 1990 resolved trades showed this book is **momentum, not mean-reversion**: the "hasn't moved our way yet" cohort (`captured_r ≤ 0`) is the WORST performer (**41.0% win / −0.40%**, n=278), while stocks already confirming the direction win MORE (0–0.25 ATR: 48.4%; 0.25–0.5: 53.8%). So skip a candidate whose price-action hasn't yet confirmed the thesis (`captured_r ≤ MOMENTUM_CONFIRM_MIN_R`). Ships **DARK** (always counts `momentum_would_skip`, only drops when enabled). Reuses the unit-tested `captured_atr()`. **FAIL-OPEN via a price-validity guard** — `captured_atr()` returns 0.0 on *both* bad data and genuinely-flat, so the gate only applies when `base/current > 0` (a glitch can never *cause* a skip). ⚠️ This is the one lever the eval data actually endorsed; `UNREACTED_GATE_ENABLED` (its opposite) was shown to drop the *best* trades — leave that OFF. |

The selection funnel (`SELECTION_FUNNEL`: `liquidity_skip` / `atr_skip` /
`unreacted_would_skip` / `unreacted_skip` / `momentum_would_skip` / `momentum_skip` /
`ensemble_rejected` / `ensemble_approved`) is
...
- **Frontend**: No deploy build step. Edit `frontend/index.html`, the `frontend/app-*.js` chunks, `frontend/styles.css` directly. Flask serves via `static_folder`. Browser refresh fetches latest.
  ...
  - **Cold-start boot reduction (backend, `app.py`).** `from google import genai` / `from google.genai import types` plus the module-level `client = _bootstrap_gemini_client()` were importing grpc/protobuf **and constructing a Gemini client at import** — ~1.5s paid on *every* gunicorn boot **before the first byte**, even though genai is only touched by AI-call sites (never first-paint, `/api/health`, or any read endpoint). Now `genai`/`types` are a transparent **lazy-module proxy** (`_LazyModule` — imports `google.genai` on first attribute access) and the client is built lazily via **`_ensure_gemini_client()`** on first AI use. The 7 global-`client` consumers (4× `client.models.generate_content`, 2× `api_client=`, 1× `active_client =`) route through it; `get_and_rotate_client()` / the per-batch `_ai_client` already self-build, and the AI-availability gate keys off `_available_gemini_key_indices()` (not `client is None`), so a lazily-`None` client never triggers the rule-based fallback. Result: `import app` **~3.5s→~1.8s** warm (larger absolute saving on a cold Render container — cold bytecode + slower CPU), shaving seconds off cold-start TTFB. Behaviour is identical the moment any AI path runs (verified: lazy proxy resolves `genai.Client`/`types.GenerateContentConfig` on demand; 51 routes; 234 tests pass). ⚠️ The free-tier **sleep-after-15-min** is still the *dominant* delay — the real cure is keeping the dyno warm (broaden the keep-alive cron beyond market hours), a Gemini-keys-vs-latency tradeoff, not a code fix.
