...
for sid, ticker, bp, cp, nt in signals:
    try:
        ...
        # We always want to fix it if the current base_price is just equal to current price (which means missed movement)
        if intraday_price > 0 and abs(intraday_price - bp) > 0.01:
            fixes.append((intraday_price, sid))
            print(f"[{ticker}] fixed! Base: {bp} -> {intraday_price}")
...
if fixes:
    ...
    conn.commit()
    print(f"Successfully fixed {len(fixes)} remaining signals.")
